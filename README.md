...
- **1st / 2nd** are that day's earliest and latest clock-in/clock-out, and
  **Hrs of work** is the sum of its paired shift(s)' durations (see "Notes on
  Hours Worked" below). A **day shift** (clock-in before 18:30) is attributed
  to the day it *started* on. A **night shift** (clock-in at/after 18:30) is
  attributed to the day it's worked *into* — the day it clocks out on — e.g.
  a shift clocking in Tuesday at 22:00 and out Wednesday at 06:00 is
  Wednesday's, including a Sunday-night shift, which moves onto Monday's row
  rather than staying on Sunday's.
- On a scheduled day, **Shift** is `N/S` (night shift) if the clock-in time
  is at/after 18:30, and `D/S` (day shift) otherwise; unscheduled days are
  `OFF` either way.
